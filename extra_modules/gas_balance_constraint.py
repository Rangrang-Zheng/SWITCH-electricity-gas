...
def define_components(m):
    ...
    m.DisposalCost = Expression(
        m.PERIODS,
        rule=lambda m, p: sum(
            m.gas_disposing_cost * (m.RelaxBalanceDown[z, ts] * m.ts_scale_to_period[ts])
            for (z, ts) in m.GAS_ZONES_TIMESERIES
            if m.ts_period[ts] == p
        ),
    )
    ...
    m.Gas_Cost_Components_Per_Period.append("RelaxCost")
    m.Gas_Cost_Components_Per_Period.append("DisposalCost")
...
def post_solve(instance, outdir):
    ...
    write_table(
        ...
        values=lambda m, p: (
            p,
            m.GasSystemCostPerPeriod[p] + m.gl_cost_adder[p],
            (m.GasSystemCostPerPeriod[p] + m.gl_cost_adder[p])*m.bring_annual_costs_to_base_year[p],
            (m.GasSystemCostPerPeriod[p] + m.gl_cost_adder[p])*m.bring_annual_costs_to_base_year[p]
            / sum(getattr(m, component)[z, ts] for component in m.Zone_Gas_Withdrawals
            ...
            sum(getattr(m, component)[z, ts] * m.ts_scale_to_period[ts] for component in m.Zone_Gas_Withdrawals
                if component in ("gas_demand_total", "FlexibleDemandTotal", "GasConsumedByElectricity") if hasattr(m, component)
                for (z, ts) in m.GAS_ZONES_TIMESERIES)
        ),
    )
