...
for i in case_list:
    ...
    # drop empty rows
    resource_capacity_agg.loc[resource_capacity_agg["end_MWh"] == ".", "end_MWh"] = 0
...
for i in case_list:
    ...
    for y in year_list:
        ...
        if y == "foresight":
            df.loc[df["PERIOD"] == 2027, "start_value"] = df["existing_trans_cap"]

            fill_dict2027 = (
                df.loc[df["PERIOD"] == 2027]
                .groupby("line_name")["TxCapacityNameplate"]
                .last()
                .to_dict()
            )
            df.loc[df["PERIOD"] == 2030, "start_value"] = df["line_name"].map(
                fill_dict2027
            )
            ...
            df.loc[df["PERIOD"] == 2035, "start_value"] = df["line_name"].map(
                fill_dict2030
            )

            fill_dict2035 = (
                df.loc[df["PERIOD"] == 2035]
                .groupby("line_name")["TxCapacityNameplate"]
                .last()
                .to_dict()
            )
            ...
            df.loc[df["PERIOD"] == 2045, "start_value"] = df["line_name"].map(
                fill_dict2040
            )

            fill_dict2045 = (
                df.loc[df["PERIOD"] == 2045]
                .groupby("line_name")["TxCapacityNameplate"]
                .last()
                .to_dict()
            )
        ...
        df["end_value"] = df["TxCapacityNameplate"].apply(lambda x: round(x, 0))
        df["start_value"] = df["start_value"].apply(lambda x: round(x, 0))
        df = df[
            [
                "model",
                "line_name",
