...
def define_components(mod):
    ...
    GASDATES denote blocks of consecutive gasdats within a period.
    # An individual gasdate could represent a single day, a week, a
    # month or an entire year. This replaces the DATE construct in the old
    # Switch code and is meant to be more versatile. The following parameters
    # describe attributes of a timeseries.

    date_period[gd] = The period a gasdate falls in.

    gd_scale_to_year[gd]: The number of times this representative
    gasdates is expected to occur in a year. Used as a scaling
    factor.

    gd_scale_to_period[gd]: The number of times this representative
    gasdates is expected to occur in a period. Used as a scaling
    factor.

    DATES_IN_PERIOD[period]: The set of gasdates in a period.
    ...
    * tp_weight[t] = 203.3 hr/period
    ...
    mod.GASDATES = Set(dimen=1, ordered=True)
    mod.dates_period = Param(mod.GASDATES, within=mod.PERIODS)
    mod.gd_scale_to_period = Param(mod.GASDATES, within=NonNegativeReals)
    ...
    # PERIOD THE DATE FALLS IN
    mod.DATES_IN_PERIOD = Set(
        mod.PERIODS,
        dimen=1,
        ordered=True,
        within=mod.GASDATES,
        initialize=lambda m, p: [d for d in m.GASDATES if m.dates_period[d] == p],
    )
    ...
    # GASDATE THE PERIOD FALLS IN
    mod.TPS_IN_GASDATES = Set(
        mod.GASDATES,
        dimen=1,
        ordered=True,
        within=mod.TIMEPOINTS,
        initialize=lambda m, d: [t for t in m.TIMEPOINTS if m.tp_date[t] == d],
    )
    ...
    mod.gd_scale_to_year = Param(
        mod.GASDATES,
        within=NonNegativeReals,
        initialize=lambda m, gd: (
            m.gd_scale_to_period[gd] / m.period_length_years[m.dates_period[gd]]
        ),
    )
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        filename=os.path.join(inputs_dir, "dates.csv"),
        index=mod.GASDATES,
        param=(mod.dates_period, mod.gd_scale_to_period),
    )
