...
def define_components(m):
    ...
    m.LNG_ROUTE_DATES = Set(
        dimen=3, initialize=lambda m: m.DIRECTIONAL_ROUTE * m.GASDATES
    )
    # m.LNG_ROUTE_DATES_ALLOWED = Set(
    #     dimen=3,
    #     initialize=m.DIRECTIONAL_ROUTE * m.GASDATES,
    #     filter=lambda m, z, p: m.LNG_flow_allowed[m.LNG_d_route[r]])

    m.LNGShipped = Var(
        m.LNG_ROUTE_DATES,  # m.LNG_ROUTE_DATES_ALLOWED
        within=NonNegativeReals,
    )
    ...
    m.LNGReceived = Expression(
        ...
        rule=lambda m, zone_from, zone_to, gd: (
            m.LNGShipped[zone_from, zone_to, gd]
    ...
    m.SumLNGReceived = Expression(
        ...
        rule=lambda m, z, gd: sum(
            m.LNGReceived[zone_from, z, gd]
            for zone_from in m.LNG_TRANSPORTIONS_TO_ZONE[z]
        ),
    )
    m.LNGLiquefiedFromNG = Var(m.GAS_ZONES_DATES, within=NonNegativeReals)
    ...
    m.LNG_Liquefied_Volume_Upper_Limit = Constraint(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.LNGLiquefiedFromNG[z, gd]
        <= m.LNGStorageCapacity[z, m.dates_period[gd]],
    )

    ### the amount of gas added to storage in each gas zone during each DATES
    m.LNGStorageAdditionQuantity = Expression(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.LNG_import_ref_quantity[z, gd]
    ...
    m.SumLNGShipped = Expression(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: sum(
            m.LNGShipped[z, zone_to, gd] for zone_to in m.LNG_TRANSPORTIONS_TO_ZONE[z]
        ),
    )
    m.LNGRegasifiedToNG = Var(m.GAS_ZONES_DATES, within=NonNegativeReals)
    ...
    m.LNG_Regasifying_Volume_Upper_Limit = Constraint(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.LNGRegasifiedToNG[z, gd]
        <= m.LNGStorageAdditionQuantity[z, gd] - m.SumLNGShipped[z, gd],
    )
    ### the amount of gas removed from storage in each gas zone during each DATES
    m.LNGStorageWithdrawalQuantity = Expression(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.SumLNGShipped[z, gd] + m.LNGRegasifiedToNG[z, gd],
    )
    ...
    ## LNG inventory
    m.LNGStorageQuantity = Var(m.GAS_ZONES_DATES, within=NonNegativeReals)

    m.ts_previous = Param(
        m.GASDATES,
        within=m.GASDATES,
        initialize=lambda m, gd: m.DATES_IN_PERIOD[m.dates_period[gd]].prevw(gd),
    )

    # track state of storage: previous quantity + injections - withdrawals
    def Track_State_Of_LNG_Storage_rule(m, z, gd):
        return m.LNGStorageQuantity[z, gd] == m.LNGStorageQuantity[
            z, m.ts_previous[gd]
        ] + (
            m.LNGStorageAdditionQuantity[z, gd] - m.LNGStorageWithdrawalQuantity[z, gd]
        )

    m.Track_State_Of_LNG_Storage = Constraint(
        m.GAS_ZONES_DATES, rule=Track_State_Of_LNG_Storage_rule
    )
    ...
    def State_Of_LNG_Storage_Upper_Limit_rule(m, z, gd):
        return (
            m.LNGStorageQuantity[z, gd] <= m.LNGStorageCapacity[z, m.dates_period[gd]]
        )

    m.State_Of_LNG_Storage_Upper_Limit = Constraint(
        m.GAS_ZONES_DATES, rule=State_Of_LNG_Storage_Upper_Limit_rule
    )
    ...
    m.LNG_Storage_Cycle_Limit = Constraint(
        ...
        (
            ...
            if m.LNG_storage_max_cycles_per_year[g] == float("inf")
                ...
                sum(
                    m.LNGStorageWithdrawalQuantity[g, gd] * m.gd_scale_to_period[gd]
    ...
    m.State_Of_LNG_Liquefaction_Upper_Limit = Constraint(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.LNGLiquefiedFromNG[z, gd]
        <= m.LNGLiqefactionCapacity[z, m.dates_period[gd]],
    )
    ...
    m.State_Of_LNG_Vaporization_Upper_Limit = Constraint(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.LNGRegasifiedToNG[z, gd]
        <= m.LNGVaporizationCapacity[z, m.dates_period[gd]],
    )
    ...
    m.LNGLiquefactionLoss = Expression(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.GasQuantityLiquefiedToLNG[z, gd]
    ...
    m.LNGRegasificationLoss = Expression(
        m.GAS_ZONES_DATES,
        rule=lambda m, z, gd: m.LNGRegasifiedToNG[z, gd] * m.LNG_to_NG_loss_ratio[z],
    )
...
def load_inputs(m, gas_switch_data, inputs_dir):
    ...
    gas_switch_data.load_aug(
        ...
        param=(m.LNG_liquefaction_predet_cap, m.LNG_liquefaction_removed_cap),
    )
    # LNG Regasification
    gas_switch_data.load_aug(
        ...
        param=(m.LNG_vaporization_predet_cap, m.LNG_vaporization_removed_cap),
    )
    ...
    gas_switch_data.load_aug(
        ...
        select=("GAS_ZONES", "DATES", "LNG_import_ref_quantity"),
