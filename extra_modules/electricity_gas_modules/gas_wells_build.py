...
def define_components(m):
    ...
    m.GAS_WELL_IN_ZONE = Set(
        dimen=2, initialize=m.GAS_ZONES * m.DRILL_TYPE
    )  # 2-dimension index(zone, drill type)
    m.PROD_YEAR = Set(dimen=1)
    m.GAS_WELL_IN_ZONE_PRODYEAR = (
        Set(  # 3-dimension index(zone, drill type, production year)
            dimen=3, initialize=m.GAS_ZONES * m.DRILL_TYPE * m.PROD_YEAR
        )
    )
    m.BLD_YRS_FOR_EXISTING_GAS_WELL = Set(
        dimen=3
    )  # 3-dimension index(zone, drill type, build year)
    m.gas_well_new_build_allowed = Param(m.GAS_ZONES, within=Boolean)
    m.gas_well_predet_num = Param(
        m.BLD_YRS_FOR_EXISTING_GAS_WELL, within=NonNegativeReals, default=0
    )
    ...
    m.gas_well_max_age = Param(m.DRILL_TYPE, within=PositiveReals, default=30)
    m.NEW_GAS_WELL_BLD_YRS = Set(
        dimen=3,
        ordered=True,
        initialize=m.GAS_WELL_IN_ZONE * m.PERIODS,
        filter=lambda m, z, dt, p: m.gas_well_new_build_allowed[z],
    )  # dt is abbreviation for Well Drill Type
    ...
    def bounds_BuildWell(model, z, dt, bld_yr):
        if (z, dt, bld_yr) in model.BLD_YRS_FOR_EXISTING_GAS_WELL:
            return (
                model.gas_well_predet_num[
                    z, dt, bld_yr
                ],  # number of gas well by drill type built at a given year
    ...
    m.BuildWellNum = Var(
        m.BLD_YRS_FOR_GAS_WELL, within=NonNegativeIntegers, bounds=bounds_BuildWell
    )
    ...
    m.max_gas_well_build_year = Param(
        m.GAS_ZONES, within=NonNegativeReals, default=0
    )  # if no data, then not build any well
    ...
    m.Gas_Well_Upper_Limit = Constraint(
        ...
        rule=lambda m, z, p: (
            ...
            if not m.gas_well_new_build_allowed[z]
                ...
                sum(m.BuildWellNum[z, dt, p] for dt in m.DRILL_TYPE)
    ...
    m.BuildWell_assign_default_value = BuildAction(
        m.BLD_YRS_FOR_EXISTING_GAS_WELL, rule=BuildWell_assign_default_value
    )
    ...
    m.BLD_YRS_FOR_WELL_PERIOD = Param(
        m.GAS_WELL_IN_ZONE,
        m.PERIODS,  # zone, drill_type, periods
        within=Any,
        initialize=lambda m, z, dt, period: set(
            bld_yr
            for (zone, drill_type, bld_yr) in m.BLD_YRS_FOR_GAS_WELL
            if drill_type == dt
            and zone == z
            and well_build_can_operate_in_period(m, dt, bld_yr, period)
        ),
    )
    m.DRILL_TYPE_OF_WELL_PERIOD = Param(
        ...
        initialize=lambda m, z, p: set(
            drill_type
            for (zone, drill_type, period) in m.GAS_WELL_IN_ZONE * m.PERIODS
    ...
    # Gas Supply Quantity is a sum of WellProductionCapacity, i.e.production capacity across wells in each zone in a given DATES
    m.GasSupplyQuantityByType = Expression(
        m.GAS_WELL_IN_ZONE,
        m.GASDATES,
        rule=lambda m, z, dt, gd: sum(
            m.WellNumInProdYear[z, dt, prod_yr, m.dates_period[gd]]
            * m.production_rate_mmbtud[z, dt, prod_yr]
            # / (24 / m.ts_duration_hrs[gd])  # convert from daily to DATES.
    ...
    m.GasSupplyQuantity = Expression(
        ...
        rule=lambda m, z, gd: sum(
            m.GasSupplyQuantityByType[z, dt, gd] for dt in m.DRILL_TYPE
        ),
    )
