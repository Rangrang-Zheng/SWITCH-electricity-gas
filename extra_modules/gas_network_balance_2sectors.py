    ...
    def Track_State_Of_Storage_rule(m, z, ty, ts):
        return (
            ...
            * (
                ...
                - (m.GasStorageInjectionQuantity[z, ty, ts] * (1 - m.gas_store_to_release_ratio[z, ty]))
            )
        )
...
#     )
    ...
    m.zone_total_gas_demand_in_period_mmbtu = Expression(
        m.GAS_ZONES,
        m.PERIODS,
        rule=lambda m, z, p: (
            sum(
                #(m.Zone_Gas_Balance[z, gd]) * m.gd_scale_to_period[gd]
                (m.gas_demand_total[z, ts]+ m.GasConsumedByElectricity[z, ts]) * m.ts_scale_to_period[ts]
                #m.gas_demand_total[z, gd] * m.gd_scale_to_period[gd]
                for ts in m.TS_IN_PERIOD[p]
            )
        ),
    )    
    ...
    m.Gas_Cost_Components_Per_TP.append('GasProdCostsPerTP')
    ...
    m.Gas_Cost_Components_Per_TP.append('TransmissionCostsPerTP')
    ...
    m.Gas_Cost_Components_Per_TP.append('StorageCostsPerTP')
    ...
    m.Gas_Cost_Components_Per_TP.append('LNGLiquefactionCostsPerTP')
    ...
    m.Gas_Cost_Components_Per_TP.append('LNGRegasificationCostsPerTP')