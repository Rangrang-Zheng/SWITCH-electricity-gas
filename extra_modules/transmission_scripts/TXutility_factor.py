...
root_folder = "/Users/rangrang/Desktop/Switch-USA-PG/RR_study/trans_study_21TW"

dispatch_list_path = [
    "decarb_hydrogen/no",
    "decarb_hydrogen/withintra",
    "decarb_hydrogen/withall",
    "decarb_hydrogen/withall_25",
    "nodecarb_hydrogen/no",
    "nodecarb_hydrogen/withintra",
    "nodecarb_hydrogen/withall",
    "nodecarb_hydrogen_co2p/no",
...
for i in dispatch_list_path:
    dispatch = pd.read_csv(os.path.join(root_folder, "out", i, "dispatch.csv"))
    df = dispatch.copy()
    ...
    tp = pd.read_csv(os.path.join(root_folder, "in", "timepoints.csv"))
    ts = pd.read_csv(os.path.join(root_folder, "in", "timeseries.csv"))
    ...
    # from TX_util import tech_type_group
    ...
    disp_mc.to_csv(os.path.join(root_folder, "results_data", i, "disp_mc.csv"))

    gen_cap = pd.read_csv(os.path.join(root_folder, "out", i, "gen_cap.csv"))
    gc = gen_cap.groupby(
    ...
    gen_info = pd.read_csv(os.path.join(root_folder, "in", "gen_info.csv"))
...
for i in path_list:
    result = [
        filename
        for filename in os.listdir(os.path.join(root_folder, "out", i))
        if filename.startswith("energy_sources")
    ]
    es = pd.read_csv(os.path.join(root_folder, "out", i, result[0]))
    es = es.loc[es["load_zone"] != "loadzone"]
    es["timeseries"] = [x[0] + "_" + x[1] for x in es["timepoint_label"].str.split("_")]
    # merge timepoint and timeseries to get tiomepoints weight
    tp = pd.read_csv(os.path.join(root_folder, "in", "timepoints.csv"))
    ...
    disp_tx = pd.read_csv(os.path.join(root_folder, "out", i, "DispatchTx.csv"))
    disp_tx["line"] = (
        disp_tx["TRANS_TIMEPOINTS_1"].astype(str)
        + " "
        + disp_tx["TRANS_TIMEPOINTS_2"].astype(str)
    ...
    tx = pd.read_csv(os.path.join(root_folder, "out", i, "transmission.csv"))
    ...
    tx_all["model"] = i
    tx_all.to_csv(
        os.path.join(root_folder, "results_data", i, "tx_rent.csv"), index=False
    )
