"""
Adapt original model for Gas-Electricity integration to include a demand response system
# Description: This module calibrates demand data and iterate over the calibration process with three demand sectors.
# In which, two sectors have demand responsive directly to price changes:
1. Industrial (EI) sector: price is defined as marginal cost and variable across timeseries-gaszones
...
# The third sector - Electricity sector - is not demand responsive to price changes, but its gas consumption is
defined through the gas-electricity integration cost minimization.
...
def define_components(m):
    ...
    m.Gas_Cost_Components_Per_TP.append("DR_Welfare_Cost")
...
def pre_iterate(m):
    if m.iteration_number == 0:
        ...
        m.prev_GasConsumedByElectricity = {
            (z, ts): 0 for z in m.GAS_ZONES for ts in m.TIMESERIES
        }
        m.prev_GasSystemCost = None
        m.prev_direct_cost_by_electricity = None
        ...
        m.prev_GasConsumedByElectricity = {
            (z, ts): value(m.GasConsumedByElectricity[z, ts])
            for z in m.GAS_ZONES
            for ts in m.TIMESERIES
        }
        m.prev_GasSystemCost = value(m.GasSystemCost)
    ...
    if m.iteration_number > 0:
        ...
        prev_direct_cost_by_electricity = sum(
            m.prev_recoverable_cost[z, ts, 'EI'] *value(m.prev_GasConsumedByElectricity[z, ts])
            * m.ts_scale_to_year[ts]
            * m.bring_annual_costs_to_base_year[m.ts_period[ts]]
            for z in m.GAS_ZONES
            for ts in m.TIMESERIES
        ...
        prev_direct_cost_by_electricity = value(prev_direct_cost_by_electricity)

        prev_welfare_cost = value(
            sum(
                #by definition: m.bring_timepoint_costs_to_base_year[tp] = m.bring_annual_costs_to_base_year[m.tp_period[t]] * m.tp_weight_in_year[t]
                m.DR_Welfare_Cost[tp] * m.bring_timepoint_costs_to_base_year[tp] 
                for tp in m.TIMEPOINTS
            )
        )
        prev_cost = prev_direct_cost + prev_welfare_cost + prev_direct_cost_by_electricity

        print("")
        print("previous direct cost: ${:,.0f}".format(prev_direct_cost))
        print("previous direct cost by electricity: ${:,.0f}".format(prev_direct_cost_by_electricity))
    ...
    if m.iteration_number > 0:
        ...
        best_direct_cost_by_electricity = sum(
            (m.prev_recoverable_cost[z, ts, 'EI'] * value(m.prev_GasConsumedByElectricity[z, ts])
             * m.ts_scale_to_year[ts]
             * m.bring_annual_costs_to_base_year[m.ts_period[ts]]
             for z in m.GAS_ZONES
        ...
        best_direct_cost_by_electricity = value(best_direct_cost_by_electricity)
        ...
        best_cost = best_direct_cost + best_bid_benefit + best_direct_cost_by_electricity

        print("")
        print("best direct cost: ${:,.0f}".format(best_direct_cost))
        print("best direct cost byelectricity: ${:,.0f}".format(best_direct_cost_by_electricity))
    ...
    converged = (
        ...
        and abs(prev_cost - best_cost) / abs(prev_direct_cost) <= 0.001 #0.0001
    )
...
def post_iterate(m):
    ...
    print("Solved model")
    print("=======================================================")
    print("Total gas cost: ${v:,.0f}".format(v=value(m.GasSystemCost)))

    GasSystemCost = value(m.GasSystemCost)  # calculate once to save time
    if m.prev_GasSystemCost is None:
        print("prev_GasSystemCost=<n/a>, GasSystemCost={:,.0f}, ratio=<n/a>".format(GasSystemCost))
    else:
        print("prev_GasSystemCost={:,.0f}, GasSystemCost={:,.0f}, ratio={}".format(
            m.prev_GasSystemCost, GasSystemCost, GasSystemCost / m.prev_GasSystemCost
        ))
...
def total_direct_costs_per_year(m, period):
    ...
    return value(
        sum(
            ...
            for annual_cost in m.Gas_Cost_Components_Per_Period
        )
        + sum(
...
def revenue_imbalance(flat_price_rc, m, gas_zone, period, recoverable_cost):
    ...
    revenue_by_electricity = 0.0
    dynamic_revenue_by_electricity = 0.0
    ...
    for ts in m.TS_IN_PERIOD[period]:
        ...
        # Electricity sector
        for z in m.GAS_ZONES:
            gas_consumed = m.prev_GasConsumedByElectricity[z, ts]
            price_e = recoverable_cost[z, ts, 'EI'][0] #set up as a float number since m.GasConsumedByElectricity is a float number
            revenue_by_electricity += price_e * gas_consumed * m.ts_scale_to_year[ts]
            
            # Calculate dynamic revenue for the electricity sector
            dynamic_revenue_by_electricity += recoverable_cost[z, ts, 'EI'][0] * gas_consumed * m.ts_scale_to_year[ts]
        
    # Calculate the total revenue and the target revenue
    total_revenue = flat_price_revenue_rc + marginal_price_revenue_ei + revenue_by_electricity
    ...
    dynamic_total_revenue = dynamic_revenue_rc + dynamic_revenue_ei + dynamic_revenue_by_electricity
...
def add_bids(m, bids):
    ...
    reconstruct(m.GasSystemCostPerPeriod)
    reconstruct(m.GasSystemCost)
...
def summary_values(m):
    ...
    values.extend(
        [
            ...
            m.GasSystemCost + sum(m.gl_cost_adder[p] * m.bring_annual_costs_to_base_year[p] for p in m.PERIODS),  
        ]
    )
        ...
        if const in duals:
            ...
            if bound is None:
                ...
                if not (-1e-3 < dual < 1e-3):
                    ...
                    #)
                    print(
                        "{} has no {} bound but has a non-zero dual value {}. Skipping...".format(
                            const.name, "lower" if dual > 0 else "upper", dual
                        )
                    )
            else:
